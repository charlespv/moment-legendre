...
/** Methode de creation d'une image Bmp "standard" (pas de LUT, pas d'alpha, codage sur 24 bits)
  ...
  */
BmpImg createBmpImg( int dimX, int dimY ) {
...
BmpImg initImg( ) {
    ...
    bmpImg.headerSize = 0;
    bmpImg.fileSize = 0;
    // 3) On fait pointer le header et les plans couleurs vers NULL
...
void readImg( FILE* fImg, BmpImg* bmpImg ) {
	...
	bmpImg->imgR = calloc( bmpImg->dimX, sizeof(unsigned char*) );
	bmpImg->imgG = calloc( bmpImg->dimX, sizeof(unsigned char*) );
	bmpImg->imgB = calloc( bmpImg->dimX, sizeof(unsigned char*) );
	for( i = 0; i < bmpImg->dimX; i++ ){
		bmpImg->imgR[i] = calloc( bmpImg->dimY, sizeof(unsigned char) );
		bmpImg->imgG[i] = calloc( bmpImg->dimY, sizeof(unsigned char) );
		bmpImg->imgB[i] = calloc( bmpImg->dimY, sizeof(unsigned char) );
	}
	...
	for( pos = 0; pos < bmpImg->imgSize; pos = pos + 3 ) {
		fread( &(bmpImg->imgB[i][j]), sizeof( unsigned char ), 1, fImg );
		fread( &(bmpImg->imgG[i][j]), sizeof( unsigned char ), 1, fImg );
		fread( &(bmpImg->imgR[i][j]), sizeof( unsigned char ), 1, fImg );
		j++;
		if( j >= bmpImg->dimY ) {
            // 4 byte alignment
...
void writeBmpImage( char* imgName, BmpImg* bmpImg ) {
	...
	for( i = bmpImg->dimX-1; i >= 0; i-- ) {
		for( j = 0; j < bmpImg->dimY; j++ ) {
			fwrite( &(bmpImg->imgB[i][j]), sizeof( unsigned char ), 1, fImg );
			fwrite( &(bmpImg->imgG[i][j]), sizeof( unsigned char ), 1, fImg );
			fwrite( &(bmpImg->imgR[i][j]), sizeof( unsigned char ), 1, fImg );
		}
        // 4 bytes alignment
        for( j = 0; j < byteAlignment; j++ ) {
...
void copyBmpHeader( BmpImg origImg, BmpImg* copyImg ) {
    // 1) On copie les dimensions de l'image
...
void copyBmpImg( BmpImg origImg, BmpImg* copyImg ) {
    ...
    if( (dimXO == dimXC) && (dimYO == dimYC) ) {
        ...
        freeMat( copyImg->imgR, dimXC );
        freeMat( copyImg->imgG, dimXC );
        freeMat( copyImg->imgB, dimXC );
        // 2) On cree les plans couleurs
        createRGBImg( copyImg );
        // 3) On alloue et copie les colonnes
        for( i = 0; i < dimXC; i++ ) {
            for( j = 0; j < dimYC; j++ ) {
                copyImg->imgR[i][j] = origImg.imgR[i][j];
                copyImg->imgG[i][j] = origImg.imgG[i][j];
                copyImg->imgB[i][j] = origImg.imgB[i][j];
            }
        }
    }
}
...
/** Methode renvoyant la couleur (r, g, b) d'un pixel
  ...
  * @return un tableau d'entier contenant la couleur RGB du pixel (0-255) ou
  * 	NULL si en dehors de l'image
  */
int* getPixel( BmpImg bmpImg, int x, int y ) {
	...
	if( (x >= 0) && (x < bmpImg.dimX) ) {
		if( (y >= 0) && (y < bmpImg.dimY) ) {
			...
			res[0] = bmpImg.imgR[x][y];
			res[1] = bmpImg.imgG[x][y];
			res[2] = bmpImg.imgB[x][y];
		}
	}
	return res;
}
...
/** Methode stockant la couleur (r, g, b) d'un pixel
  * @param bmpImg une structure image bmp
  * @param clr un tableau d'entier contenant la couleur RGB du pixel (0-255)
  * @param x la coordonnee x (ligne) du pixel
  * @param y la coordonnee y (colonne) du pixel
  */
void setPixel( BmpImg bmpImg, int* clr, int x, int y ) {
	if( (x >= 0) && (x < bmpImg.dimX) ) {
		if( (y >= 0) && (y < bmpImg.dimY) ) {
			bmpImg.imgR[x][y] = (unsigned char)(clr[0]);
			bmpImg.imgG[x][y] = (unsigned char)(clr[1]);
			bmpImg.imgB[x][y] = (unsigned char)(clr[2]);
		}
	}
}
...
/** Methode allouant l'espace memoire necessaire aux 3 plans couleur d'une image Bmp
  ...
  */
void createRGBImg( BmpImg* bmpImg ) {
